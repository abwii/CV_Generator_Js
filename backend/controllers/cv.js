...
} = require("../validator/cvDateConverter");
const {
  convertToDateObject,
  formatUpdatedCV,
  formatCVResponse,
...
module.exports = {
  ...
  findCVByUserId: (req, res) => {
    const userId = req.params.id; // Récupère l'ID utilisateur à partir des paramètres de la requête

    CVModel.findOne({ user: userId }) // Cherche un CV correspondant à l'ID utilisateur
      .populate("user", "firstname lastname email phone address") // Peupler les détails de l'utilisateur référencé dans le CV
      .then((cv) => {
        if (!cv) {
          // Si aucun CV n'est trouvé pour cet utilisateur
          return res.status(404).send({
            message: `CV not found for user with id=${userId}`,
          });
        }
        res.send(cv); // Renvoie le CV trouvé
      })
      .catch((error) => {
        // En cas d'erreur, renvoie un message d'erreur
        res.status(500).send({
          message:
            error.message || `Error retrieving CV for user with id=${userId}`,
        });
      });
  },
  ...
  updateMyCV: async (req, res) => {
    try {
      ...
      if (
        newCV.skills &&
        JSON.stringify(newCV.skills) !== JSON.stringify(existingCV.skills)
      ) {
        updateFields.skills = newCV.skills;
      }
      if (
        newCV.skills &&
        JSON.stringify(newCV.softSkills) !==
          JSON.stringify(existingCV.softSkills)
      ) {
        updateFields.softSkills = newCV.softSkills;
      }
      if (
        newCV.skills &&
        JSON.stringify(newCV.languages) !== JSON.stringify(existingCV.languages)
      ) {
      ...
      if (newCV.education) {
        existingCV.education = existingCV.education.filter((existingEdu) =>
          newCV.education.some(
            (newEdu) => newEdu._id && newEdu._id.equals(existingEdu._id)
          )
        );

        newCV.education.forEach((newEdu) => {
          const existingEdu = existingCV.education.find(
            (existingEdu) =>
              existingEdu._id && existingEdu._id.equals(newEdu._id)
          );

          if (existingEdu) {
            const startDateToCheck = newEdu.startDate
              ? newEdu.startDate
              : existingEdu.startDate;
            const endDateToCheck = newEdu.endDate
              ? newEdu.endDate
              : existingEdu.endDate;
            ...
            existingEdu.degree =
              newEdu.degree !== undefined ? newEdu.degree : existingEdu.degree;
            existingEdu.institution =
              newEdu.institution !== undefined
                ? newEdu.institution
                : existingEdu.institution;
            existingEdu.startDate = convertToDateObject(newEdu.startDate);
            existingEdu.endDate = convertToDateObject(newEdu.endDate);
            existingEdu.description =
              newEdu.description !== undefined
                ? newEdu.description
                : existingEdu.description;
          } else {
      ...
      if (newCV.experience) {
        existingCV.experience = existingCV.experience.filter((existingExp) =>
          newCV.experience.some(
            (newExp) => newExp._id && newExp._id.equals(existingExp._id)
          )
        );

        newExperience.forEach((newExp) => {
          const existingExp = existingExperience.find(
            (existingExp) =>
              existingExp._id && existingExp._id.equals(newExp._id)
          );

          if (existingExp) {
            const startDateToCheck = newExp.startDate
              ? newExp.startDate
              : existingExp.startDate;
            const endDateToCheck = newExp.endDate
              ? newExp.endDate
              : existingExp.endDate;
            ...
            existingExp.title =
              newExp.title !== undefined ? newExp.title : existingExp.title;
            existingExp.company =
              newExp.company !== undefined
                ? newExp.company
                : existingExp.company;
  ...
  deleteMyCV: (req, res) => {
    ...
    CVModel.findByIdAndDelete(cvId)
      ...
      .catch((error) => {
        res
          .status(500)
          .send(error.message || `Cannot delete CV with id=${cvId}`);
      });
  },
};
